@extends('developer.store.innerStore.frame')
...
@section('storeContents')
  ...
  <div class="storeAdd stores">
    ...
    @endif
    <h2 class="mb-4">Tag Informations</h2>
    <form action="{{ route('adminTagSubmit') }}" method="post" enctype="multipart/form-data">
      @csrf
      <div class="form-row">
        <div class="form-group col-md-6">
          <label for="name">Tag Name (Required)</label>
          <input type="text" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" id="title" name="name" onkeyup="url_gen()" value="{{ old('name') }}" placeholder="Please insert Tag name." required autofocus>
          @if ($errors->has('name'))
      ...
      <div class="form-group">
        <label for="url">Tag Url (Required)</label>
