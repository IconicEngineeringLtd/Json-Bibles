      ...
      <div class="col-md-9 mainContents">
        ...
        <div class="row productShow">
          <div class="col-md-6 leftSide">
            <div class="mainImage">
              ...
              @if(App\Wishlist::where('user_id', Auth::id())->where('product_id', $product->id)->orWhere('ipAddress', $_SERVER['REMOTE_ADDR'])->where('product_id', $product->id)->exists())
          ...
          <div class="col-md-6 rightSide">
            ...
            <p>Brand: <strong>{{ucwords($product->brand->name)}}</strong></p>
            <p>Category: <a href="{{route('category_product', $product->category->url)}}" class="text-accent"><strong>{{ucwords($product->category->name)}}</strong></a></p>
            <p>Sub Category: <a href="{{route('subCategoryProduct', ['category_url'=> $product->category->url, 'subcategory_url'=> $product->sub_category->url])}}" class="text-accent"><strong>{{ucwords($product->sub_category->name)}}</strong></a></p>
            ...
            <!-- Add to cart start -->
            @php
            if(App\Cart::where('user_id', Auth::id())->where('product_id', $product->id)->orWhere('ipAddress', $_SERVER['REMOTE_ADDR'])->where('product_id', $product->id)->exists()){
              $quantity = App\Cart::where('user_id', Auth::id())->where('product_id', $product->id)->orWhere('ipAddress', $_SERVER['REMOTE_ADDR'])->where('product_id', $product->id)->first()->quantity;
            }else{
              $quantity = 1;
            }
            @endphp
            <form action="{{route('addCartProduct')}}" method="post" class="d-inline">
              @csrf
              <p>Quantity: </p><input class="my-2 d-block quantity form-control text-accent" type="number" name="quantity" autocomplete="off" value="{{$quantity}}">
            ...
            <form action="{{route('addFavoriteProduct')}}" method="post" class="d-inline">
              @csrf
              <button class="btn bg-accent" type="submit" name="product_id" value="{{$product->id}}">
                @if(App\Wishlist::where('user_id', Auth::id())->where('product_id', $product->id)->orWhere('ipAddress', $_SERVER['REMOTE_ADDR'])->where('product_id', $product->id)->exists())
            ...
            @auth
              @if(!empty(Auth::user()->myStore) && Auth::user()->myStore->id == $product->store->id || Auth::user()->role == 1)
                ...
                <div class="border mt-5 p-1" title="Only Store owner can see this section.">
                  ...
                  <a href="{{route('supplierEditProduct', $product->url)}}" class="mt-1 d-block"><i class="fas fa-edit"></i> Edit This Product</a>
        ...
        <div class="relatedProducts d-none d-md-block">
          ...
          <div class="row <?php echo(count($viewedProducts) >= 5)?'productSlider':'';?>">
            @foreach($viewedProducts->take(5) as $viewedproduct)
              @php
              $viewedproduct = $viewedproduct->productInfo;
              @endphp
            <div class="col-md-3 px-1 mb-1">
              <a href="{{ route('product_details', $viewedproduct->url) }}" class="card text-dark">
                ...
                <div class="card-body">
                  <h5 class="card-title">{{str_limit($viewedproduct->title, 15, '..')}}</h5>
        ...
        <!-- Mobile Version -->
        <div class="relatedProducts d-block d-md-none">
          <h5 class="header">Products you viewed</h5>
        ...
        <div class="suggestProducts">
          ...
          <a class="media pt-2 pr-1" href="{{ route('product_details', $product->url) }}">
            ...
            <div class="media-body">
              <h6 class="mt-0 text-accent">{{ str_limit($product->title, 20, '..') }}</h6>
              <p>{!! strip_tags(str_limit($product->overview, 35, '..')) !!}</p>
            </div>
          </a>
          @endforeach
        </div>
